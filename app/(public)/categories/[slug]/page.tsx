...
import { getCategoryBySlug, getPublishedBlogsByCategory } from '@/lib/supabase/taxonomy';
...
export default async function CategoryDetailPage({ params }: CategoryDetailPageProps) {
  ...
  const blogs = await getPublishedBlogsByCategory(category.id);
        ...
        <section className="space-y-4">
          ...
          <div className="max-w-3xl">
            ...
            <p className="mt-3 text-base leading-7 text-zinc-300">
              {category.description || '该分类下的已发布文章'}
            </p>
          </div>
        </section>
        ...
        {blogs.length === 0 ? (
          <section className="flex min-h-[360px] items-center justify-center">
            <div className="text-center space-y-4">
              ...
              <p className="text-zinc-400 max-w-md">该分类下暂时没有已发布文章。</p>
            </div>
          </section>
        ) : (
