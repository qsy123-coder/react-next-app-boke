import Link from 'next/link';
import { BookOpen, ArrowRight } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { getCategories } from '@/lib/supabase/taxonomy';

export default async function CategoriesPage() {
  const categories = await getCategories();

  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(244,114,182,0.18),_transparent_24%),radial-gradient(circle_at_bottom_right,_rgba(56,189,248,0.18),_transparent_22%),#09090b] px-4 py-28 text-white sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl space-y-10">
        <section className="space-y-4">
          <Badge className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-white backdrop-blur" variant="outline">
            分类
          </Badge>
          <div className="max-w-3xl">
            <h1 className="text-4xl font-semibold tracking-tight sm:text-5xl">文章分类</h1>
            <p className="mt-3 text-base leading-7 text-zinc-300">
              按主题浏览所有文章分类
            </p>
          </div>
        </section>

        {categories.length === 0 ? (
          <section className="flex min-h-[400px] items-center justify-center">
            <div className="text-center space-y-4">
              <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-white/5 backdrop-blur">
                <BookOpen className="h-10 w-10 text-zinc-400" />
              </div>
              <h2 className="text-2xl font-semibold text-white">暂无分类</h2>
              <p className="text-zinc-400 max-w-md">创建文章时添加分类后，这里会展示所有分类。</p>
            </div>
          </section>
        ) : (
          <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={`/categories/${category.slug}`}
                className="group rounded-[28px] border border-white/10 bg-white/5 p-6 backdrop-blur transition hover:bg-white/10"
              >
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="text-xl font-semibold text-white">{category.name}</h2>
                    <p className="mt-2 line-clamp-2 text-sm leading-6 text-zinc-400">
                      {category.description || '探索该分类下的文章'}
                    </p>
                  </div>
                  <ArrowRight className="h-5 w-5 text-zinc-500 transition group-hover:translate-x-1 group-hover:text-white" />
                </div>
              </Link>
            ))}
          </section>
        )}
      </div>
    </main>
  );
}
